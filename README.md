...
* 유니코드 키의 특징
    ...
    `문자가 아닌 키 경우에는 표준화된 키 코드 값 없음 <Function> 키, <Home> 키, <up> 키, <delete> 키, <Control> 키,
    <Shift> 키, <Alt> 등은 플랫폼에 따라 키 코드 값이 다를 수 있음`
...
* int KeyEvent.getKeyCode()
    ...
    -입력된 키를 판별하기 위해 가상 키(Virtual Key) 값과 비교하여야 함
...
* 컨테이너에 새로운 배치관리자 설정
    ...
    -seLayout(LayoutManager lm) 메소드 호출 : lm을 새로운 배치관리자로 설정
...
* 배치관리자가 없는 컨테이너가 필요한 경우
    ...
    -게임 프로그램과 같이 시간이나 마우스/키보드의 입력에 따라 컴포넌트를의 위치와 크기가 수시로 변하는 경우
